...
const newsReducer = (state: NewsState, action: NewsAction): NewsState => {
  switch (action.type) {
    case 'SET_ARTICLES':
      return { ...state, articles: action.payload, filteredArticles: action.payload };
    case 'SET_LOADING':
    ...
    case 'SET_CATEGORY': {
      ...
      const searchFiltered = state.searchQuery 
        ? filtered.filter(a => a.title.toLowerCase().includes(state.searchQuery.toLowerCase())) 
        : filtered;
    ...
    case 'SET_SEARCH': {
      ...
      const finalFiltered = categoryFiltered.filter(a => 
