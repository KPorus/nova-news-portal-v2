...
const CATEGORIES: Category[] = ['All', 'National', 'International', 'Sports', 'Technology', 'Business', 'Entertainment'];
    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Logo Section */}
          <div className="flex-shrink-0 flex items-center cursor-pointer" onClick={() => handleCategoryClick('All')}>
            ...
            {CATEGORIES.slice(0, 5).map((cat) => (
              <button
                ...
                className={`text-sm font-medium transition-colors hover:text-primary-600 ${
                  state.selectedCategory === cat ? 'text-primary-600 border-b-2 border-primary-600' : 'text-gray-600'
                }`}
              >
                {cat}
              </button>
            ...
            <div className={`relative transition-all duration-300 ${isSearchOpen ? 'w-64' : 'w-8'}`}>
              ...
              <input
                ...
                className={`w-full pl-8 pr-4 py-1 text-sm border-b-2 focus:outline-none bg-transparent ${
                  isSearchOpen ? 'border-primary-600 opacity-100' : 'border-transparent opacity-0 pointer-events-none'
                }`}
